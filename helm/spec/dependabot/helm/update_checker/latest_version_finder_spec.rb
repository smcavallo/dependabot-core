...
require "dependabot/helm/update_checker/latest_version_finder"

RSpec.describe Dependabot::Helm::UpdateChecker::LatestVersionFinder do
  ...
  describe "#latest_version" do
    context "when there's a newer major version but not a new minor version" do
      ...
      it "returns the latest minor version for the dependency's current major version" do
        expect(finder.latest_version).to eq(Dependabot::Helm::Version.new("1.1.0"))
      end
      ...
      context "with an unrestricted goprivate" do
        ...
        it "returns the latest minor version for the dependency's current major version" do
          ...
          expect(finder.latest_version).instance_of?(Dependabot::Helm::Version)
    ...
    context "when already on the latest version" do
      ...
      it "returns the current version" do
        expect(finder.latest_version).to eq(Dependabot::Helm::Version.new("3.0.0"))
      end
    end
    ...
    context "with a go.mod excluded version" do
      ...
      it "doesn't return to the excluded version" do
        expect(finder.latest_version).to eq(Dependabot::Helm::Version.new("1.0.6"))
      end
    end
    ...
    context "with Dependabot-ignored versions" do
      ...
      it "doesn't return Dependabot-ignored versions" do
        expect(finder.latest_version).to eq(Dependabot::Helm::Version.new("1.0.1"))
      end
    end
    ...
    context "when on a pre-release" do
      ...
      it "returns newest pre-release" do
        expect(finder.latest_version).to eq(Dependabot::Helm::Version.new("1.2.0-pre2"))
      end
    end
    ...
    context "for a Git psuedo-version with releases available" do
      ...
      it "picks the latest version not ignored" do
        expect(finder.latest_version).to eq(Dependabot::Helm::Version.new("0.8.0"))
      end
    end
    ...
    context "when the latest version is an '+incompatible' version" do # https://golang.org/ref/mod#incompatible-versions
      ...
      it "returns the current version" do
        expect(finder.latest_version).to eq(Dependabot::HelmCharts::Version.new("2.0.0+incompatible"))
      end
    end
  ...
  describe "#lowest_security_fix_version" do
    ...
    context "when on a stable release and a newer versions are available" do
      it "returns the lowest available new release" do
        expect(finder.lowest_security_fix_version).to eq(Dependabot::HelmCharts::Version.new("1.0.1"))
      end
    end
    ...
    context "with Dependabot-ignored versions" do
      ...
      it "doesn't return Dependabot-ignored versions" do
        expect(finder.lowest_security_fix_version).to eq(Dependabot::HelmCharts::Version.new("1.0.5"))
      end
    end
    ...
    context "with a go.mod vulnerable version" do
      ...
      it "doesn't return to the vulnerable version" do
        expect(finder.lowest_security_fix_version).to eq(Dependabot::HelmCharts::Version.new("1.0.6"))
      end
    end
    ...
    context "with a Git pseudo-version and releases available" do
      ...
      it "picks the minimum version that isn't vulnerable" do
        expect(finder.lowest_security_fix_version).to eq(Dependabot::HelmCharts::Version.new("0.6.0"))
      end
      ...
      context "with a pseudo-version as the patched version" do
        ...
        it "picks the minimum version that is safe" do
          expect(finder.lowest_security_fix_version).to eq(Dependabot::HelmCharts::Version.new("0.1.0"))
        end
      end
    end
    ...
    context "when on a pre-release" do
      ...
      it "returns newest pre-release" do
        expect(finder.lowest_security_fix_version).to eq(Dependabot::HelmCharts::Version.new("1.2.0-pre2"))
      end
    end
    ...
    context "when on a stable release and a newer prerelease is available" do
      ...
      it "doesn't return pre-release" do
        expect(finder.lowest_security_fix_version).to_not eq(Dependabot::HelmCharts::Version.new("1.2.0-pre2"))
      end
    end
  end
end
