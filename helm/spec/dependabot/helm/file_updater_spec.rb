...
RSpec.describe Dependabot::Helm::FileUpdater do
  ...
  let(:files) { [chart_yaml, chart_lock] }
  ...
  let(:chart_yaml) do
    Dependabot::DependencyFile.new(name: "Chart.yaml", content: chart_yaml_body)
  end
  let(:go_mod_body) { fixture("projects", project_name, "go.mod") }

  let(:chart_lock) do
    Dependabot::DependencyFile.new(name: "Chart.lock", content: chart_lock_body)
  end
  let(:chart_lock_body) { fixture("projects", project_name, "Chart.lock") }
  ...
  describe "#updated_dependency_files" do
    ...
    context "with an invalid module path" do
      ...
      it "raises a helpful error" do
        expect { updated_files }.to raise_error(Dependabot::HelmChartPathMismatch)
      end
    end
    ...
    context "without a clone of the repository" do
      ...
      it "disables the tidy option" do
        double = instance_double(
          Dependabot::HelmCharts::FileUpdater::HelmChartUpdater,
          updated_go_mod_content: "",
          updated_go_sum_content: ""
        )

        expect(Dependabot::HelmCharts::FileUpdater::HelmChartUpdater)
          .to receive(:new)
          .with(
