...
RSpec.describe Dependabot::Helm::FileUpdater::ChartUpdater do
  ...
  let(:go_mod_content) { fixture("projects", project_name, "Chart.yaml") }
  let(:tidy) { true }
  let(:directory) { "/" }
  let(:goprivate) { "*" }
  ...
  describe "#updated_chart_yaml_content" do
    subject(:updated_chart_yaml_content) { updater.updated_chart_yaml_content }
      ...
      let(:dependency_files) do
        [
          Dependabot::DependencyFile.new(
            ...
            content: chart_yaml_content.gsub("rsc.io/qr v0.1.0", "rsc.io/qr v0.1.1")
          )
        ]
      end
    ...
    context "for a top level dependency" do
      ...
      context "if no files have changed" do
        it { is_expected.to eq(chart_yaml_content) }
      end
      ...
      context "when the requirement has changed" do
        ...
        context "for a go 1.11 Chart.yaml" do
        ...
        context "for a go 1.12 Chart.yaml" do
          let(:project_name) { "simple" }
        ...
        describe "a dependency who's module path has changed during an update" do
          ...
          it "raises the correct error" do
            error_class = Dependabot::HelmChartPathMismatch
        ...
        context "with a Chart.lock" do
          let(:project_name) { "chart_lock" }
          subject(:updated_chart_yaml_content) { updater.updated_chart_lock_content }

          it "adds new entries to the Chart.lock" do
            is_expected
          ...
          it "removes old entries from the Chart.lock" do
            is_expected
          ...
          describe "a non-existent dependency with a pseudo-version" do
            ...
            it "raises the correct error" do
              ...
              expect { updater.updated_chart_lock_content }
                .to raise_error(error_class) do |error|
          ...
          describe "a dependency that has no top-level package" do
            ...
            it "does not raise an error" do
              expect { updater.updated_chart_lock_content }.to_not raise_error
            end
          end

          describe "with a Chart.yaml that is not in the root directory" do
            let(:project_name) { "not_root" }

            it "updates the Chart.yaml" do
              expect(updater.updated_chart_yaml_content).to include(
                %(rsc.io/quote v1.5.2\n)
              )
            end
            ...
            it "does not leave a temporary file lingering in the repo" do
              updater.updated_chart_yaml_content
        ...
        context "without a Chart.lock" do
          let(:project_name) { "simple" }

          it "doesn't return a Chart.lock" do
  ...
  describe "#updated_chart_lock_content" do
    let(:project_name) { "chart_lock" }
    subject(:updated_chart_yaml_content) { updater.updated_chart_lock_content }
    ...
    context "for a top level dependency" do
      ...
      context "if no files have changed" do
        let(:chart_lock_content) { fixture("projects", project_name, "Chart.lock") }
        it { is_expected.to eq(chart_lock_content) }
      end
